...
parser.add_argument('--save-moving', required=True, help='moving image (source) filename')
parser.add_argument('--save-fixed', required=True, help='fixed image (target) filename')
...
moving, fixed = vxm.py.utils.load_bl_fu_file(args.moving, test_augment, add_batch_axis=True,
...
vxm.py.utils.save_volfile(input_moving, args.save_moving)

input_fixed = input_fixed.detach().cpu().numpy().squeeze()
vxm.py.utils.save_volfile(input_fixed, args.save_fixed)
...
# save warp
if args.warp:
    warp = warp.detach().cpu().permute(0, 2, 3, 4, 1).numpy().squeeze()
