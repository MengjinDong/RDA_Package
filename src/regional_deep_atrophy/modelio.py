...
def store_config_args(func):
    ...
    attrs = [p.name for p in params.values() if p.kind in (p.POSITIONAL_OR_KEYWORD,)]

    # Get varargs (*args)
    varargs = [p.name for p in params.values() if p.kind == p.VAR_POSITIONAL]

    # Get varkw (**kwargs)
    varkw = [p.name for p in params.values() if p.kind == p.VAR_KEYWORD]
